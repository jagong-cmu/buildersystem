...
export function ScrollGuide({ initial }: { initial: Manual }) {
  ...
  const status = step ? (verify[step.n]?.status ?? "pending") : "pending";
    ...
    <div className="stage">
      ...
      <div className="absolute inset-x-0 top-0 p-4 flex items-start gap-3 pointer-events-none">
        ...
        <div className="ml-auto flex items-center gap-2 pointer-events-auto">
          <button className={`btn sm${live ? " primary" : ""}`} onClick={() => setLive((l) => !l)}>{live ? "hide live" : "live"}</button>
      ...
      {live && (
        <div className="live-pip shadow-xl">
          <LiveFeed compact />
        </div>
      )}
