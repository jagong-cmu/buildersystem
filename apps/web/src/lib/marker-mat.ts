...
export function markerMatSvg(markerSvg: (id: number) => { svg: string; size: number }): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}mm" height="${H}mm" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="white"/>${grid.join("")}${markers.join("")}<text x="${W / 2}" y="${H / 2 - 4}" font-size="8" text-anchor="middle" fill="#999" font-family="sans-serif">Dojo · scrap mat</text><text x="${W / 2}" y="${H / 2 + 6}" font-size="5" text-anchor="middle" fill="#999" font-family="sans-serif">Print on A3 at 100 % (no scaling). Grid 10 mm. Markers ${MAT.dictionary} ${M} mm.</text></svg>`;
}
