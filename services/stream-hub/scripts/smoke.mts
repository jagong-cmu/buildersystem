...
async function main() {
  if (process.argv.includes("--loop")) {
    const hub = process.env.HUB ?? "ws://127.0.0.1:8799";
    const ws = new WebSocket(`${hub}/produce?source=test&kind=phone`);
    ...
    for (;;) {
      seq++;
      ws.send(encodeFrame({ v: 1, sourceId: "test", seq, ts: Date.now(), w: 1, h: 1, mime: "image/jpeg" }, jpeg));
      if (seq % 10 === 0) console.log("sent:", seq);
